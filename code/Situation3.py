...
device = torch.device(
    "cuda:%s" % hvd.local_rank() if torch.cuda.is_available() else "cpu"
)
...
style_dataset = torchvision.datasets.ImageFolder(
    "/home/ypw/WikiArt/", transform=data_transform
)
content_dataset = torchvision.datasets.ImageFolder(
    "/home/ypw/COCO/", transform=data_transform
)
...
visualization_content_images = torch.stack(
    [random.choice(content_dataset)[0] for i in range(4)]
).to(device)
...
for epoch in range(epochs):
    ...
    with tqdm(enumerate(content_data_loader), total=n_batch) as pbar:
        for batch, (content_images, _) in pbar:
            ...
            # content loss
            content_loss = content_weight * F.mse_loss(
                transformed_features[2], content_features[2]
            )

            # style loss
            style_loss = style_weight * F.mse_loss(
                transformed_mean_std, style_mean_std.expand_as(transformed_mean_std)
            )
            ...
            if (batch + 1) % verbose_image_batch == 0:
                ...
                visualization_transformed_images = transform_net(
                    visualization_content_images
                )
                transform_net.train()
                visualization_transformed_images = torch.cat(
                    [style_image, visualization_transformed_images]
                )
                writer.add_image(
                    "debug", recover_tensor(visualization_transformed_images), n_iter
                )
    ...
    if not is_hvd or hvd.rank() == 0:
        torch.save(
            metanet.state_dict(), "checkpoints/{}_{}.pth".format(model_name, epoch + 1)
        )
        torch.save(
        ...
        torch.save(
            transform_net.state_dict(), "models/{}_transform_net.pth".format(model_name)
        )
