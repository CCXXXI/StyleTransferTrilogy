...
def recover_image(tensor):
    ...
    image = image * np.array(cnn_normalization_std).reshape((1, 3, 1, 1)) + np.array(
        cnn_normalization_mean
    ).reshape((1, 3, 1, 1))
    return (image.transpose(0, 2, 3, 1) * 255.0).clip(0, 255).astype(np.uint8)[0]
...
def mean_std(features):
    ...
    for x in features:
        ...
        n = x.shape[0]
        x2 = (
            x.view(n, 2, -1).transpose(2, 1).contiguous().view(n, -1)
